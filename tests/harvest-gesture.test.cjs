...
function setup(){
 ...
 const ctx={handlers,harvests,console,Math,Set,Map,setTimeout:()=>{},clearTimeout:()=>{},document:{addEventListener:(name,fn)=>handlers[name]=fn},window:{addEventListener:()=>{}},$:id=>els[id]||{},modalType:null,gesture:null,seedPlot:2,mode:'browse',pointers:new Map(),camera:{},storeBubble:{hidden:true},state:{tutorial:'done',plots:[{plant:{remaining:0}},{plant:{remaining:3}},{plant:{remaining:0}},{plant:null}]},G:{plotAt:x=>Math.floor(x/100),stage:p=>p},mapPoint:e=>({x:e.clientX,y:e.clientY}),closeSeeds:()=>{ctx.closed=true;ctx.seedPlot=null;},render:()=>{},doHarvest:i=>{harvests.push(i);ctx.state.plots[i].plant=null;},ghost:()=>{},levelPopup:()=>{},plotClick:()=>{},applyCamera:()=>{}};
