...
assets={p.relative_to(root).as_posix():'data:image/png;base64,'+base64.b64encode(p.read_bytes()).decode() for p in (root/'assets/coral').rglob('*.png') if p.parent.name!='animated' or p.relative_to(root).as_posix() in active_atlases}
html=(root/'index.html').read_text()
html=re.sub(r'src="(assets/[^\"]+)"',lambda m:'src="'+assets[m[1]]+'"',html)
html=re.sub(r'<link rel="stylesheet" href="style.css[^"]*">','<style>'+(root/'style.css').read_text()+'</style>',html)
