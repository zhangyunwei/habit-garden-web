 ...
 function sync(){
  ...
  for(const info of G.animals){const id=info.id;if(!G.active(s,id))continue;
   let v=views.get(id);if(!v){
    ...
    const fallback=new Image();fallback.onload=()=>{if(views.has(id))paint(id,views.get(id),true);};fallback.src=asset(`assets/coral/sprites/animals/animal_${id}_idle.png`);
